defmodule PrxAuth do
  ...
  def authorized_resources(%PrxAuth.User{} = user, scope) do
    Enum.filter(Map.keys(user.auths), fn resource_id -> is_authorized?(user, resource_id, scope) end) 
  end

  def authorized_resources(%PrxAuth.User{} = user, namespace, scope) do
    Enum.filter(Map.keys(user.auths), fn resource_id -> is_authorized?(user, resource_id, namespace, scope) end)
  end
end
